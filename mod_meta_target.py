...
class MarkerReLoad(object):
    ...
    def __init__(self):
        ...
        self.configModule()
        self.moduleMarker()
        BigWorld.logInfo(self.name, 'mod_meta_target is started', None)
    ...
    def moduleMarker(self):
        ...
        def checkCollides():
            for id in self.visible_list:
                if not __getIsFriendly(id):
                    if isCollide(id):
                        if id not in self.edged_list:
                            addEdge(id)
                            self.edged_list.append(id)
                            #debug only
                        ...
                        if id in self.edged_list:
                            delEdge(id)
                            self.edged_list.remove(id)
                            #debug only
                            veh = BigWorld.player().arena.vehicles.get(id)
