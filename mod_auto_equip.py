...
def equip_optional_devices(vehicle):
    if g_settings[vehicle.name]:
        for slot_idx in range(0, 3):
            device = vehicle.descriptor.optionalDevices[slot_idx]
            if not device:
                device_id = read_device(vehicle, slot_idx)
                if device_id != -1:
                    BigWorld.player().inventory.equipOptionalDevice(vehicle.invID, device_id, slot_idx, False, None)
                    LOG_DEBUG('equip: %s, slot=%d, id=%d' % (vehicle.name, slot_idx, device_id))

def remove_optional_devices(vehicle):
...
def equip_current_vehicle():
    if g_currentVehicle.isInHangar():
        ...
        remove_optional_devices_from_all_vehicles(current)
...
def new_AmmunitionPanel_as_setDataS(self, data):
    ...
    LOG_DEBUG('devices on %s:' % vehicle.name)
    for info in data['devices']:
        if info['slotType'] == 'optionalDevice':
            LOG_DEBUG(' id=%d, slot=%d, removable=%s' % (info['id'], info['slotIndex'], info['removable']))
            slot_idx = info['slotIndex']
            device_id = info['id']
            if info['removable']:
                if device_id != -1 and device_id != read_device(vehicle, slot_idx):
